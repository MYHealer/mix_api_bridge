...
async fn chat_completions(ctrl: Arc<ProxyController>, body: Value) -> Response {
    ...
    // Route OpenCode free models to the OpenCode upstream. Unlike mimo, OpenCode
    // requires no auth, session management, or response normalization — just a
    // raw HTTP passthrough with SSE streaming.
    if !model_req.is_empty() && ctrl.is_opencode_model(&model_req) {
        return opencode_forward(ctrl, body).await;
    }
