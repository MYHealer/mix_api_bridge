...
/// Forward a request to the OpenCode upstream, streaming the response back.
/// Unlike the mimo forwarder, there is no authentication, session refresh, or
/// cookie management — just a plain HTTP passthrough to opencode.ai.
pub async fn opencode_forward(ctrl: Arc<ProxyController>, body: Value) -> Response {
    let model = body
        .get("model")
        .and_then(|v| v.as_str())
        .unwrap_or("")
