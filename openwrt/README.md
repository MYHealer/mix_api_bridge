# mix_api_bridge on OpenWrt (LuCI 插件)

把 `mix_api_bridge` 的 headless server 跑在路由器上，并在 LuCI 后台里通过
...
└───────────────┬────────────────────────────────┘
                │ 同源相对请求 /api /v1
                ▼
   mix_api_bridge server  (procd 常驻, :8765, 0.0.0.0)
                │
                ▼
...
- OpenWrt 没有 Secret Service / keyring，用现成的环境变量
  `MICLAW_API_BRIDGE_DISABLE_KEYRING=1` 走磁盘存储，session 落在 `/etc/mix_api_bridge`。
...
- `openwrt/out/luci-app-mix_0.1.0-1_aarch64_cortex-a53.ipk` —— **可直接 `opkg install` 的安装包**（推荐）
- `openwrt/out/mix_api_bridge` —— aarch64 静态二进制（已 strip，约 9.5M）
- `openwrt/out/mix_api_bridge_openwrt_aarch64.tar.gz` —— tarball + `install.sh`（不想用 opkg 时的备选）
...
scp openwrt/out/luci-app-mix_0.1.0-1_aarch64_cortex-a53.ipk $ROUTER:/tmp/
ssh $ROUTER 'opkg install /tmp/luci-app-mix_0.1.0-1_aarch64_cortex-a53.ipk'
```

opkg 会自动：装好全部文件、跑 `postinst`（enable + start 服务、刷新 LuCI 缓存）。
`/etc/config/mix_api_bridge` 标记为 conffile，升级时不会覆盖你的改动。

卸载：`opkg remove luci-app-mix`（`prerm` 会先停服务）。
...
ipk 已把 `/etc/config/mix_api_bridge` 设为 conffile（固件升级保留）。登录态存在
`/etc/mix_api_bridge/`，要保留它，把这行加进 `/etc/sysupgrade.conf`：
...
（`/etc/mix_api_bridge/config/mix_api_bridge/tls-cert.pem`）。

```bash
uci set mix_api_bridge.main.tls=1
uci commit mix_api_bridge
/etc/init.d/mix_api_bridge restart
```
...
```bash
scp root@192.168.2.1:/etc/mix_api_bridge/config/mix_api_bridge/tls-cert.pem .
# macOS：加入系统钥匙串并信任
...
> `/etc/mix_api_bridge/config/mix_api_bridge/tls-cert.pem` 和 `tls-key.pem`
> 再 `/etc/init.d/mix_api_bridge restart`，会重新生成带新 SAN 的证书。
...
```bash
uci set mix_api_bridge.main.tls=1
uci set mix_api_bridge.main.tls_cert='/etc/mix_api_bridge/cert.pem'
uci set mix_api_bridge.main.tls_key='/etc/mix_api_bridge/key.pem'
uci commit mix_api_bridge
/etc/init.d/mix_api_bridge restart
```
...
scp openwrt/out/mix_api_bridge_openwrt_aarch64.tar.gz $ROUTER:/tmp/
ssh $ROUTER
# --- 以下在路由器上 ---
cd /tmp
tar -xzf mix_api_bridge_openwrt_aarch64.tar.gz
cd mix_api_bridge_openwrt
sh install.sh
```
...
# 1. 二进制
scp mix_api_bridge $ROUTER:/usr/bin/mix_api_bridge
ssh $ROUTER 'chmod +x /usr/bin/mix_api_bridge'

# 2. procd 服务 + UCI 配置
scp files/etc/init.d/mix_api_bridge $ROUTER:/etc/init.d/mix_api_bridge
ssh $ROUTER 'chmod +x /etc/init.d/mix_api_bridge'
scp files/etc/config/mix_api_bridge $ROUTER:/etc/config/mix_api_bridge

# 3. 启动并设开机自启
ssh $ROUTER '/etc/init.d/mix_api_bridge enable; /etc/init.d/mix_api_bridge start'
```
...
scp luci/root/usr/share/luci/menu.d/luci-app-mix.json \
    $ROUTER:/usr/share/luci/menu.d/luci-app-mix.json

# 3. ACL（授予读 uci / 调 service list 权限）
ssh $ROUTER 'mkdir -p /usr/share/rpcd/acl.d'
scp luci/root/usr/share/rpcd/acl.d/luci-app-mix.json \
    $ROUTER:/usr/share/rpcd/acl.d/luci-app-mix.json
...
cargo build --release --no-default-features \
  --target aarch64-unknown-linux-musl --bin mix_api_bridge

# 产物
ls -lh target/aarch64-unknown-linux-musl/release/mix_api_bridge

# 然后手动构建 ipk（不依赖 SDK）
cd ..
bash openwrt/build-ipk.sh src-tauri/target/aarch64-unknown-linux-musl/release/mix_api_bridge
```
...
`openwrt/ipk/luci-app-mix/Makefile` 是一个**可选**的 OpenWrt SDK Makefile 骨架，
