...
#
# Default output: openwrt/out/mix_api_bridge_openwrt_aarch64.tar.gz
set -euo pipefail

BIN="${1:?usage: pack-bundle.sh <binary> [output.tar.gz]}"
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
OWRT="$ROOT/openwrt"
OUT="${2:-$OWRT/out/mix_api_bridge_openwrt_aarch64.tar.gz}"

test -f "$BIN" || { echo "!! binary not found: $BIN" >&2; exit 1; }

STAGE="$(mktemp -d)"
trap 'rm -rf "$STAGE"' EXIT
PKG="$STAGE/mix_api_bridge_openwrt"
mkdir -p "$PKG"

# binary
cp -f "$BIN" "$PKG/mix_api_bridge"
chmod +x "$PKG/mix_api_bridge"
...
chmod +x "$PKG/install.sh" "$PKG/uninstall.sh" \
	"$PKG/files/etc/init.d/mix_api_bridge"

mkdir -p "$(dirname "$OUT")"
tar -czf "$OUT" -C "$STAGE" mix_api_bridge_openwrt
