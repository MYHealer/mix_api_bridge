...
function bridgePort() {
	var p = uci.get('mix_api_bridge', 'main', 'port');
	p = parseInt(p, 10);
...
function bridgeRunning(serviceState) {
	try {
		var inst = serviceState['mix_api_bridge']['instances'];
		for (var k in inst)
...
return view.extend({
	load: function() {
		return Promise.all([
			uci.load('mix_api_bridge').catch(function() { return null; }),
			callServiceList('mix_api_bridge').catch(function() { return {}; })
		]);
	},
	...
	render: function(data) {
		...
		if (!running) {
			nodes.push(E('div', { 'class': 'alert-message warning' }, [
				E('p', {}, _('The mix_api_bridge service is not running. Start it with ' +
					'"/etc/init.d/mix_api_bridge start" or enable it on boot.'))
			]));
		}
