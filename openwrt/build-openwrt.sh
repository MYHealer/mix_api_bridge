#!/usr/bin/env bash
# Cross-compile the headless mix_api_bridge server for OpenWrt / mt7981
...
#
# Output:
#   openwrt/out/mix_api_bridge   (aarch64 static, stripped)
set -euo pipefail
...
# expose the host-built ../dist (inlined by rust-embed) via the MIX_DIST_DIR
# volume declared in src-tauri/Cross.toml.
export MIX_DIST_DIR="$ROOT/dist"

build_with_cross() {
  echo "==> cross build for $TARGET (docker)"
  cross build --release --no-default-features --target "$TARGET" --bin mix_api_bridge
}
...
build_with_rustup() {
  ...
  cargo build --release --no-default-features --target "$TARGET" --bin mix_api_bridge
}
...
BIN="$ROOT/src-tauri/target/$TARGET/release/mix_api_bridge"
test -f "$BIN" || { echo "!! build did not produce $BIN" >&2; exit 1; }

# --- 3. Strip + stage -------------------------------------------------------
cp -f "$BIN" "$OUT_DIR/mix_api_bridge"
if command -v aarch64-linux-musl-strip >/dev/null 2>&1; then
  aarch64-linux-musl-strip "$OUT_DIR/mix_api_bridge" || true
elif command -v llvm-strip >/dev/null 2>&1; then
  llvm-strip "$OUT_DIR/mix_api_bridge" || true
fi

echo "==> binary: $OUT_DIR/mix_api_bridge"
file "$OUT_DIR/mix_api_bridge" 2>/dev/null || true
ls -lh "$OUT_DIR/mix_api_bridge"

# --- 4. Assemble deployable bundle -----------------------------------------
echo "==> packing deployable bundle (tar.gz)"
bash "$ROOT/openwrt/pack-bundle.sh" "$OUT_DIR/mix_api_bridge"

# --- 5. Build opkg-installable ipk -----------------------------------------
echo "==> building ipk"
bash "$ROOT/openwrt/build-ipk.sh" "$OUT_DIR/mix_api_bridge"

echo
echo "==> done. Two ways to deploy:"
echo
echo "  A) opkg package (recommended):"
echo "     scp $OUT_DIR/luci-app-mix_*_aarch64_cortex-a53.ipk root@192.168.1.1:/tmp/"
echo "     ssh root@192.168.1.1 'opkg install /tmp/luci-app-mix_*_aarch64_cortex-a53.ipk'"
echo
echo "  B) tarball + installer script:"
echo "     scp $OUT_DIR/mix_api_bridge_openwrt_aarch64.tar.gz root@192.168.1.1:/tmp/"
echo "     ssh root@192.168.1.1"
echo "     cd /tmp && tar -xzf mix_api_bridge_openwrt_aarch64.tar.gz && cd mix_api_bridge_openwrt && sh install.sh"
