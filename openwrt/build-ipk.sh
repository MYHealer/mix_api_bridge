...
#
# This packs ONE merged package `luci-app-mix` containing the server binary,
...
install -m 0644 "$OWRT/luci/root/usr/share/rpcd/acl.d/luci-app-mix.json" \
  "$DATA/usr/share/rpcd/acl.d/luci-app-mix.json"
...
Maintainer: neoruaa
Description: LuCI app + headless server for mix_api_bridge (Mimo Bridge).
...
/etc/config/mix_api_bridge
EOF

cat > "$CTRL/postinst" <<'EOF'
#!/bin/sh
[ -n "${IPKG_INSTROOT}" ] && exit 0
if [ -x /etc/init.d/mix_api_bridge ]; then
	/etc/init.d/mix_api_bridge enable 2>/dev/null
	/etc/init.d/mix_api_bridge start 2>/dev/null
fi
...
[ -n "${IPKG_INSTROOT}" ] && exit 0
if [ -x /etc/init.d/mix_api_bridge ]; then
	/etc/init.d/mix_api_bridge stop 2>/dev/null
	/etc/init.d/mix_api_bridge disable 2>/dev/null
fi
exit 0
EOF
